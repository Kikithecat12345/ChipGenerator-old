/* numbers.js
 * This file contains all number-related functions.
 * any and all user-editable variables are at the top of the file.
 */
  ...
  for (let i = prefixNumSections.length - 1; i >= 0; i--) {
    ...
    if (prefixNumSections[i] == 0) {
      ...
      if (prefixNumSections[i] >= 100) {
        ...
        lastPrefixAdded =
          hundredsPrefixes[Math.floor(prefixNumSections[i] / 100) - 1]; // minus one cause index 0 is 1
      ...
      if (prefixNumSections[i] % 100 >= 10) {
        // add the tens prefix
        lastPrefixAdded =
          tensPrefixes[Math.floor((prefixNumSections[i] % 100) / 10) - 1];
        prefix = lastPrefixAdded + prefix;
      }
